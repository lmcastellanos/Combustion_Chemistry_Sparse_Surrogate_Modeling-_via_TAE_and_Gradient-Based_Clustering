...
plt.tight_layout()
plt.savefig('interpolation_r2_scores_cluster_3.jpg',
            dpi=300, bbox_inches='tight')  # High-res output only at save time

scores=pd.DataFrame(scores)
scores.to_csv(join(cluster_name,'cluster3_scores.csv'))

j=13 #equivalence ratio to be checked 
...
coefficients.to_csv(join(cluster_name,'cluster3_coefficients.csv'))
intercepts.to_csv(join(cluster_name,'cluster3_intercepts.csv'))

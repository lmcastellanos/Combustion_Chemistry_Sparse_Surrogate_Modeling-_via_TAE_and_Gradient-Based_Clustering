...
phis=np.array([1.25,1.35,1.45,1.55])
...
plt.tight_layout()
plt.savefig('r2_scores_cluster_2.jpg',
            dpi=300, bbox_inches='tight')  # High-res output only at save time

scores=pd.DataFrame(scores)
scores.to_csv(join(cluster_name,'cluster2_scores.csv'))

j=10 #equivalence ratio to be checked 
...
coefficients.to_csv(join(cluster_name,'cluster2_coefficients.csv'))
intercepts.to_csv(join(cluster_name,'cluster2_intercepts.csv'))

...
phis=np.array([0.45,0.55, 0.65,0.75, 0.85, 0.95,1.05, 1.15,1.25])
...
latent_features=['OH', 'CH3OH','CH3', 'H', 'CH2OH','CH4', 'HO2', 'CO', 'O2','CO2','H2','C','HCO', 'HCCO', 'Phi']
...
plt.tight_layout()
plt.savefig(join(cluster_name,'r2_scores_cluster_0.jpg'),
            dpi=300, bbox_inches='tight')  # High-res output only at save time

scores=pd.DataFrame(scores)
scores.to_csv(join(cluster_name,'cluster0_scores.csv'))

j=8 #equivalence ratio to be checked 
...
coefficients.to_csv(join(cluster_name,'cluster0_coefficients.csv'))
intercepts.to_csv(join(cluster_name,'cluster0_intercepts.csv'))
